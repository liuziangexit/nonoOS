...
function plotEllipseRect(x0, y0, x1, y1) {                              /* rectangular parameter enclosing the ellipse */
...
function plotQuadBezierSeg(x0, y0, x1, y1, x2, y2) {                                  /* plot a limited quadratic Bezier segment */
...
function plotQuadRationalBezierSeg(x0, y0, x1, y1, x2, y2, w) {                   /* plot a limited rational Bezier segment, squared weight */
...
function plotQuadRationalBezier(x0, y0, x1, y1, x2, y2, w) {                                 /* plot any quadratic rational Bezier curve */
...
function plotRotatedEllipse(x, y, a, b, angle) {                                   /* plot ellipse rotated by angle (radian) */
    var xd = a * a, yd = b * b;
...
function plotRotatedEllipseRect(x0, y0, x1, y1, zd) {                  /* rectangle enclosing the ellipse, integer rotation angle */
...
function plotCubicBezierSeg(x0, y0, x1, y1, x2, y2, x3, y3) {                                        /* plot limited cubic Bezier segment */
    var f, fx, fy, leg = 1;
...
function plotCubicBezier(x0, y0, x1, y1, x2, y2, x3, y3) {                                              /* plot any cubic Bezier curve */
    var n = 0, i = 0;
...
function plotLineAA(x0, y0, x1, y1) {             /* draw a black (0) anti-aliased line on white (255) background */
    var dx = Math.abs(x1 - x0), sx = x0 < x1 ? 1 : -1;
    var dy = Math.abs(y1 - y0), sy = y0 < y1 ? 1 : -1;
...
function plotCircleAA(xm, ym, r) {                     /* draw a black anti-aliased circle on white background */
...
function plotEllipseRectAA(x0, y0, x1, y1) {        /* draw a black anti-aliased rectangular ellipse on white background */
    var a = Math.abs(x1 - x0), b = Math.abs(y1 - y0), b1 = b & 1;        /* diameter */
...
function plotQuadBezierSegAA(x0, y0, x1, y1, x2, y2) {                    /* draw an limited anti-aliased quadratic Bezier segment */
    var sx = x2 - x1, sy = y2 - y1;
    var xx = x0 - x1, yy = y0 - y1, xy;              /* relative values for checks */
...
function plotQuadBezierAA(x0, y0, x1, y1, x2, y2) {                             /* plot any anti-aliased quadratic Bezier curve */
    var x = x0 - x1, y = y0 - y1, t = x0 - 2 * x1 + x2, r;
...
function plotQuadRationalBezierSegAA(x0, y0, x1, y1, x2, y2, w) {   /* draw an anti-aliased rational quadratic Bezier segment, squared weight */
    var sx = x2 - x1, sy = y2 - y1;                  /* relative values for checks */
    var dx = x0 - x2, dy = y0 - y2, xx = x0 - x1, yy = y0 - y1;
...
function plotQuadRationalBezierAA(x0, y0, x1, y1, x2, y2, w) {                    /* plot any anti-aliased quadratic rational Bezier curve */
    var x = x0 - 2 * x1 + x2, y = y0 - 2 * y1 + y2;
    var xx = x0 - x1, yy = y0 - y1, ww, t, q;
...
function plotRotatedEllipseAA(x, y, a, b, angle) {                                   /* plot ellipse rotated by angle (radian) */
    var xd = a * a, yd = b * b;
    var s = Math.sin(angle), zd = (xd - yd) * s;               /* ellipse rotation */
    xd = Math.sqrt(xd - zd * s), yd = Math.sqrt(yd + zd * s);       /* surrounding rect*/
    a = Math.floor(xd + 0.5); b = Math.floor(yd + 0.5); zd = zd * a * b / (xd * yd);
...
function plotRotatedEllipseRectAA(x0, y0, x1, y1, zd) {                  /* rectangle enclosing the ellipse, integer rotation angle */
    var xd = x1 - x0, yd = y1 - y0, w = xd * yd;
    if (w != 0.0) w = (w - zd) / (w + w);                    /* squared weight of P1 */
    assert(w <= 1.0 && w >= 0.0);                /* limit angle to |zd|<=xd*yd */
    xd = Math.floor(xd * w + 0.5); yd = Math.floor(yd * w + 0.5);       /* snap to int */
...
function plotCubicBezierSegAA(x0, y0, x1, y1, x2, y2, x3, y3) {                           /* plot limited anti-aliased cubic Bezier segment */
    var f, fx, fy, leg = 1;
    var sx = x0 < x3 ? 1 : -1, sy = y0 < y3 ? 1 : -1;        /* step direction */
    var xc = -Math.abs(x0 + x1 - x2 - x3), xa = xc - 4 * sx * (x1 - x2), xb = sx * (x0 - x1 - x2 + x3);
    var yc = -Math.abs(y0 + y1 - y2 - y3), ya = yc - 4 * sy * (y1 - y2), yb = sy * (y0 - y1 - y2 + y3);
...
function plotCubicBezierAA(x0, y0, x1, y1, x2, y2, x3, y3) {                                              /* plot any cubic Bezier curve */
    var n = 0, i = 0;
    var xc = x0 + x1 - x2 - x3, xa = xc - 4 * (x1 - x2);
    var xb = x0 - x1 - x2 + x3, xd = xb + 4 * (x1 + x2);
    var yc = y0 + y1 - y2 - y3, ya = yc - 4 * (y1 - y2);
...
function plotLineWidth(x0, y0, x1, y1, th) {                              /* plot an anti-aliased line of width th pixel */
    var dx = Math.abs(x1 - x0), sx = x0 < x1 ? 1 : -1;
    var dy = Math.abs(y1 - y0), sy = y0 < y1 ? 1 : -1;
...
function plotEllipseRectWidth(x0, y0, x1, y1, th) {               /* draw anti-aliased ellipse inside rectangle with thick line */
...
function plotQuadRationalBezierWidthSeg(x0, y0, x1, y1, x2, y2, w, th) {   /* plot a limited rational Bezier segment of thickness th, squared weight */
    var sx = x2 - x1, sy = y2 - y1;                  /* relative values for checks */
    var dx = x0 - x2, dy = y0 - y2, xx = x0 - x1, yy = y0 - y1;
...
function plotQuadRationalBezierWidth(x0, y0, x1, y1, x2, y2, w, th) {                    /* plot any anti-aliased quadratic rational Bezier curve */
    var x = x0 - 2 * x1 + x2, y = y0 - 2 * y1 + y2;
    var xx = x0 - x1, yy = y0 - y1, ww, t, q;
...
function plotRotatedEllipseWidth(x, y, a, b, angle, th) {                                   /* plot ellipse rotated by angle (radian) */
    var xd = a * a, yd = b * b;
    var s = Math.sin(angle), zd = (xd - yd) * s;               /* ellipse rotation */
    xd = Math.sqrt(xd - zd * s), yd = Math.sqrt(yd + zd * s);       /* surrounding rect*/
    a = Math.floor(xd + 0.5); b = Math.floor(yd + 0.5); zd = zd * a * b / (xd * yd);
...
function plotRotatedEllipseRectWidth(x0, y0, x1, y1, zd, th) {                  /* rectangle enclosing the ellipse, integer rotation angle */
    var xd = x1 - x0, yd = y1 - y0, w = xd * yd;
    if (w != 0.0) w = (w - zd) / (w + w);                    /* squared weight of P1 */
    assert(w <= 1.0 && w >= 0.0);                /* limit angle to |zd|<=xd*yd */
    xd = Math.floor(xd * w + 0.5); yd = Math.floor(yd * w + 0.5);       /* snap to int */
...
function plotCubicBezierWidth(x0, y0, x1, y1, x2, y2, x3, y3, th) {                                              /* plot any cubic Bezier curve */
    var n = 0, i = 0;
    var xc = x0 + x1 - x2 - x3, xa = xc - 4 * (x1 - x2);
    var xb = x0 - x1 - x2 + x3, xd = xb + 4 * (x1 + x2);
    var yc = y0 + y1 - y2 - y3, ya = yc - 4 * (y1 - y2);
...
function plotCubicBezierSegWidth(x0, y0, x1, y1, x2, y2, x3, y3, th) {                     /* split cubic Bezier segment in two quadratic segments */
