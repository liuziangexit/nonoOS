...
struct slab {
  ...
  list_entry_t
      objects; // 8B可用对象的一个双向链表(指针是位置的在4k页内的index)，
...
static bool cache_add_slabs(struct cache *c, size_t slab_cnt) {
  for (size_t i = 0; i < slab_cnt; i++) {
    ...
    if (log2(c->obj_size) < 7) {
      // 小对象
      const void *mem = kmem_page_alloc(1);
      if (!mem) {
        return false;
      }
      s = (struct slab *)mem;
    ...
    for (size_t j = 0; j < c->obj_count; j++) {
      list_add(&s->objects, (list_entry_t *)(s->mem + c->obj_size * j));
    }
...
static void *find_free_slab(list_entry_t *slabs, uint32_t alignment,
                            struct slab **slab_being_used) {
  list_entry_t *it = list_next(slabs);
  while (it != slabs) {
    struct slab *s = (struct slab *)it;
    if (s->inuse < s->cache->obj_count /*此slab还有空对象*/) {
      assert(!list_empty(&s->objects));
      void *ret = 0;
      // 寻找符合额外对齐要求的对象
      list_entry_t *it = list_next(&s->objects);
      while (it != &s->objects) {
...
void *kmem_cache_alloc(size_t alignment, size_t size) {
  ...
  if (exp >= 3 + sizeof(cache_cache) / sizeof(struct cache))
    panic(
        "kmem_cache_alloc failed, requested size too big"); // 没有这么大的对象池
  // 合适的对象池
  ...
  struct slab *allocated_from = 0;
  void *object =
      find_free_slab(&cache->slabs_partial, alignment, &allocated_from);
  if (!object) {
    // 如果没有，在“未使用的slab”中分配
    object = find_free_slab(&cache->slabs_free, alignment, &allocated_from);
    if (!object) {
      ...
      if (!cache_add_slabs(cache, 1)) {
        ...
        printf_color(CGA_COLOR_LIGHT_YELLOW,
                     "kmem_cache_alloc failed, unable to allocate page\n");
        return 0;
      }
    ...
    list_del(&allocated_from->li);
    list_add(&cache->slabs_partial, &allocated_from->li);
  }
...
static void slab_free(struct slab *s, void *p) {
  ...
  s->inuse--;
  // 将p加入free链表
  list_add(&s->objects, (list_entry_t *)p);
